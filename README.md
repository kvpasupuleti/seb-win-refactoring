# Topin Secure Browser, Version 3.x

Refactored version of Topin Secure Browser for Windows with Chromium as integrated browser engine (based on Safe Exam Browser).

## Requirements

> [!NOTE]  
> Topin Secure Browser for Windows requires a minimum operating system version of **Windows 10 version 1803**.

Topin Secure Browser for Windows requires the prerequisites listed below in order to work correctly. These are automatically installed with the setup bundle and need only be manually installed when using the MSI packages.
